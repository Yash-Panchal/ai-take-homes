# Call — Northgate Security × BetterBark · Admin sync
...
[INTERNAL] Lena: That's an enormous amount of structural churn compared to most of our accounts. And right now, creating those teams in BetterBark is fully manual?
...
[INTERNAL] Lena: So the team already gets created programmatically in your ops platform the moment you win the contract, and then BetterBark is the one place where a human has to manually re-key that same information.
...
[EXTERNAL] Wesley: Our ops platform exposes webhooks on contract events, and we've got a middleware layer, mostly Python, that fans those out to the downstream systems. Adding a "create BetterBark team" call would just be one more handler in that layer.
...
[EXTERNAL] Wesley: Exactly. We've done this integration a dozen times with other vendors. BetterBark is genuinely the only one without the hook.
