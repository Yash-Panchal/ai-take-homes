# Call — Corvus Media × BetterBark · Program Review
...
[EXTERNAL] Aisha: Turbulent is generous. We've had two rounds of restructuring in eighteen months, and the toll that takes on people is exactly what's on my mind for this call. Which, spoiler, is where BetterBark comes in.
...
[EXTERNAL] Aisha: Brutal question. And here's my problem. I know BetterBark is sitting on an incredibly rich signal about how engaged our people are — whether they're showing up for the program, whether their dogs are actually making progress.
[INTERNAL] Sam: In what sense — the training sessions themselves?
[EXTERNAL] Aisha: Exactly. My members are booking sessions, working through real training milestones with their coaches, sticking with it or dropping off. That participation is a pulse on the organization.
...
[EXTERNAL] Aisha: The tension is this. I do not want to know what any individual is working on with their coach. That's sacred, it's confidential.
[INTERNAL] Sam: Agreed, and I'd push back hard if you did want that.
[EXTERNAL] Aisha: Good, you should. The second my people think their training sessions feed a report to leadership, engagement dies overnight and deservedly so.
[INTERNAL] Sam: So the individual level is off the table entirely. What would you want instead?
[EXTERNAL] Aisha: I would give a lot for an anonymized, aggregate, org-level read. Something like: "program participation across the org trended down 8% this quarter, concentrated in these broad functions." Trends, not individuals. Direction, not detail.
[INTERNAL] Sam: So what you're describing is a program-wellbeing trend dashboard at the executive level — participation and progress milestones, aggregate only, anonymized by design, that shows leadership the direction the organization is moving without ever exposing a single individual's data.
...
[EXTERNAL] Nathan: The big one — ideally it's derived from something the members opt into knowing is aggregated, not scraped silently from private training-session content. The provenance has to be clean or I won't put my name on it.
...
[EXTERNAL] Nathan: Right. There's a version of this that's an ethical disaster — mining private training-session notes for executive dashboards — and there's a version that's genuinely valuable and defensible. We only want the second one. If it can only be built the first way, we don't want it at all.
...
[INTERNAL] Sam: I would not have said that, and if a vendor does say that to you, run. To be clear about where we are today: we have manager and admin dashboards that show engagement and utilization — who's booking, completion rates, activity. What we do not have is an aggregate program-wellbeing trend view at the executive altitude, built to your privacy constraints. So this is a genuine feature request, not a setting.
